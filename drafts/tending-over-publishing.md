---
title: "Make publishing boring so tending stays cheap"
...
matters, but because every gate you put in front of publishing gets paid
*again* on every revision — and revision is the whole point of a garden.
...
But in a garden the first publish is the cheapest thing that will ever happen
to an article. If tending it three months later means re-running that same
...
So the test for any piece of publishing machinery is not "how long does the
first publish take?" It's "what does the four-hundredth revision cost?"
...
- **No draft state.** A draft/published boundary is a decision, and decisions
  are the expensive part. Everything here is public from the moment it exists;
  the stage field carries the "how finished is this" signal instead.
- **No manual dates.** An `updated` field in frontmatter is a thing you can
  forget, and a date you can forget is a date nobody should trust. It comes
  from the last commit that touched the file, so it cannot be wrong.

```bash
# the entire publishing pipeline
git add drafts/some-idea.md && git commit -m "tend" && git push
```

## Where the cost actually went

It didn't disappear — it moved into the build. Deriving dates from git,
inverting the link graph for backlinks, generating the raw markdown siblings
and the history documents: all of that is real complexity, and it all runs
...
[a garden is a publishing model](./example-article.md) — that one makes the
case for revising in place, and this one is about making sure revising in
place stays free.
